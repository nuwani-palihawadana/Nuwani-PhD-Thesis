...
To test the coverage of the derived prediction intervals, we calculate the average coverage of multiple prediction intervals constructed under a generic online learning framework as proposed by @Wang2024. 
...
Here we use the conformal prediction methods for computing prediction intervals with time series cross-validation forecasting proposed by @Wang2024. The workflow can be summarised as follows.

#### Conformal Prediction with Time Series Cross-validation Forecasting

1. Sequentially split the data into non-overlapping training set ($\bm{z}_{1}, \dots, \bm{z}_{tr}$), calibration set ($\bm{z}_{tr + 1}, \dots, \bm{z}_{tc}$), and test set ($\bm{z}_{tc + 1}, \dots, \bm{z}_{T}$) as in block bootstrapping. 
