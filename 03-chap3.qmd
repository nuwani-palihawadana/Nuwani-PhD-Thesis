# Estimating Uncertainty of High-dimensional Nonparametric Forecasts {#sec-PI}
...
$$ {#eq-modelform}
where $y_{t}$ is the observed value of the response variable at time $t$, $f$ is any arbitrary function, $\bm{x}_{t}$ is a vector of predictors at time $t$, and $\varepsilon_{t}$ is an i.i.d. random error. Notice that several lagged values of $y_{t}$ along with the lagged predictors are included in the model to capture any serial correlation present in the data. We also define $\bm{z}_{t} = (y_{t}, \bm{x}_{t})$, to be the observation at time $t$. 

We sequentially split the data into non-overlapping training set $\{\bm{z}_{1}, \dots, \bm{z}_{tr}\}$, calibration set $\{\bm{z}_{tr + 1}, \dots, \bm{z}_{tc}\}$, and test set $\{\bm{z}_{tc + 1}, \dots, \bm{z}_{T}\}$, and estimate the forecasting model (@eq-modelform) using the training set. Let the estimated model be
$$
  \hat{y}_{t} = \hat{f}(\bm{x}_{t}, \bm{x}_{t-1}, \dots,\bm{x}_{t-p}, y_{t-1},\dots,y_{t-k}),
$$ {#eq-estmodel}
where $\hat{y}_{t}$ is the forecast for $y_{t}$ obtained through the estimated model, $\hat{f}$ is the estimated forecasting function. Then the model residuals are given by $e_{t} = y_{t} - \hat{y}_{t}$.

To test the coverage of the derived prediction intervals from various methods, we calculate the average coverage of multiple prediction intervals constructed under a generic online learning framework as proposed by @Wang2024. 
...
Once the forecasting model given in @eq-estmodel is estimated, we obtain the series of in-sample residuals $e_{1}, \dots, e_{tr}$. Then we randomly re-sample segments/blocks of residuals, and join them together to construct new simulated residual series. If the residual series is identified as a non-seasonal time series, we use the Autocorrelation Function (ACF) to determine the size of a block. For example, if the $i^{th}$ lag of ACF is significant, we take the block size to be $i$, as this will ensure that we capture any important autocorrelation structures in the residual series. If the residual series is seasonal, we consider the *single season block bootstrap* method [@HF2010; @Politis2001 (missing reference)], where the length of each block is taken to be a multiple of the seasonal period of the series. 
...
[XW: Is the calibration set necessary here? My understanding is that we only use in-sample residuals for the block bootstrap method. Therefore, only the training set (which includes both the initial training set $\{\bm{z}_{1}, \dots, \bm{z}_{tr}\}$ and the calibration set $\{\bm{z}_{tr + 1}, \dots, \bm{z}_{tc}\}$) and the test set are required. I have totally rewritten the procedure below, let me know if you disagree with it.]{style="color:red;"}

1. Sequentially split the data into non-overlapping training set $\{\bm{z}_{1}, \dots, \bm{z}_{tc}\}$ and test set $\{\bm{z}_{tc + 1}, \dots, \bm{z}_{T}\}$.

2. Train a forecasting model on the the training set.

3. Obtain the series of in-sample residuals $\{(e_{1}, \dots, e_{tr})$.

4. Perform block bootstrapping by randomly re-sampling blocks of a selected length from the residuals series, and generate several bootstrapped series of residuals.

5. Obtain $H$-step-ahead simulated future values, $\hat{y}_{tc + 1 \mid tc}, \dots, \hat{y}_{tc + H \mid tc}$, for each bootstrapped series.
  -   ***If no response lags were used as predictors to the forecasting model:*** add bootstrapped residuals to the forecasts obtained to derive simulated future sample paths.
  ...
  -   ***If response lags were used as predictors to the forecasting model:*** perform recursive forecasting. 
      ...
      ii. Add the first element of each bootstrapped series to $1$-step-ahead forecast to obtain $1$-step-ahead simulated future values.
      ...
      iv. Repeat the process iteratively until desired forecasting horizon $H$ is reached.

6. Calculate required quantiles of the sets of simulated future values at each forecast horizon to construct prediction intervals.

7. Recursively roll the training set forward by one observation, and then repeat steps 2-6 until prediction intervals have been constructed for the entire test set.

### Prediction Intervals through Conformal Prediction {#sec-cpflow2}

Here we use the conformal prediction methods for computing prediction intervals under the online learning framework proposed by @Wang2024. The workflow can be summarised as follows.

#### Conformal Prediction with Time Series Cross-validation Forecasting

1. Sequentially split the data into non-overlapping training set ($\bm{z}_{1}, \dots, \bm{z}_{tr}$), calibration set ($\bm{z}_{tr + 1}, \dots, \bm{z}_{tc}$), and test set ($\bm{z}_{tc + 1}, \dots, \bm{z}_{T}$).

2. Perform cross-validation forecasting. Start by estimating the forecasting model on the training set, and then iteratively roll the training window forward by one observation, while repeating the model estimation until the end of the training window reaches the time point $tc - H + 1$, where $H$ is the forecasting horizon of interest.
...
4. Apply the different conformal prediction methods: SCP, WSCP, ACP, PID and MCP, to compute $H$-step prediction intervals based on the forecasting origin $tc$, using the point forecasts and non-conformity scores obtained in step 3.

5. Iteratively roll the training set and calibration set forward by one observation to update the nonconformity scores, and then repeat step 4 until prediction intervals have been constructed for the entire test set.
...
1. Sequentially split the data into non-overlapping training set ($\bm{z}_{1}, \dots, \bm{z}_{tr}$), calibration set ($\bm{z}_{tr + 1}, \dots, \bm{z}_{tc}$), and test set ($\bm{z}_{tc + 1}, \dots, \bm{z}_{T}$) as in previous methods. 
